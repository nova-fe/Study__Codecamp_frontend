...
export default function StaticRoutingMovedPage() {
  ...
  // _.debounce: 특정 이벤트가 연속적으로 발생할 때 마지막 이벤트만 실행되도록 지연
  // -> 여기서의 경우 500ms 동안 추가 호출이 없을 때(입력이 없을 때) 실행됨
  const getDebounce = _.debounce((value) => {
    setKeyword(value);
    if(!value.trim()) {   // trim() 메서드 앞뒤 공백 제거 → 빈 문자열인지 확인하기 위해 사용
    ...
    const filteredKeys = keyList.filter((key) => {
      ...
      const isMatch = title.toLowerCase().includes(value.toLowerCase()); // 해당 타이틀이 검색어를 포함하고 있는지 확인
      return isMatch; // true나 bool로 return -> true인 경우만 filteredKeys 배열에 담음
    });

    setFilteredKeyList(filteredKeys); // 필터링된 key 목록 업데이트
    setIsFilteredEmpty(filteredKeys.length === 0);
  }, 500);

  // 검색어 입력
  const onChangeSearch = (event) => {
    // 이벤트가 연속으로 발생시 마지막 이벤트만 실행되도록 지연
  ...
  return (
    <div>
      검색어입력: <input className="border" type='text' onChange={onChangeSearch}/>
        ...
        data?.map((el, index) => (
          <div key={index}>
            <span style={{ margin: '10px', fontWeight: 'bold' }}>
              {/* .replaceAll() : 철수(검색어 예시)라는걸 전부 앞과 뒤에 @#$ 라는 임의의 비밀코드로 감싼 것으로 교체함 
                  .split() : 그걸 @#$ 로 또 나눠서 배열로 만들고( ["@#$", "철수", "@#$"] )
                  .map() : 그 배열을 순회하면서, 철수(검색어) 일 때에만 span 태그에 red로 스타일 지정
              */}
              {/* {el?.writer.replaceAll("철수", "@#$철수!@#$").split("@#$").map((el, index) => {
                <span key={`${el}_${index}`} style={{color: el === "철수" ? "red" : "black"}}>{el}</span>
              })} */}
              
              {el?.title.replaceAll(keyword, `@#$${keyword}@#$`).split("@#$").map((el, index) => {
                return <span key={`${el}_${index}`} style={{color: el === keyword ? "red" : "black"}}>{el}</span>
              })}            
            </span>
