...
export default function StaticRoutingMovedPage() {
  ...
  const onChangeSearch = (event) => {
    // setSearch(event.target.value);
      ...
      if(!event.target.value.trim()) {   // trim() 메서드 앞뒤 공백 제거 → 빈 문자열인지 확인하기 위해 사용
      ...
      const filteredKeys = keyList.filter((key) => {
        ...
        const isMatch = title.toLowerCase().includes(event.target.value.toLowerCase()); // 해당 타이틀이 검색어를 포함하고 있는지 확인
